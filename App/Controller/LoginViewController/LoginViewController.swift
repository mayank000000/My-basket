                    ...
                    if arrayUsers.count>0{
                        ...
                        if user.password == self.textFieldPassword.text!{
                           HelperFunction.helper.storeInUserDefaultForKey(name: "email", val: self.textFieldEmail.text!)
                            HelperFunction.helper.storeInUserDefaultForKey(name: "username", val: user.firstName!)
                            if user.type == "User"{
                                let homeVC = loadVC(storyboardMain, viewImageDisplayerVC) as! ImageDisplayerViewController
                                                          self.navigationController?.pushViewController(homeVC, animated: true)
                            }else if user.type == "Employee"{
                                let homeVC = loadVC(storyboardMain, "OrderListViewController") as! OrderListViewController
                                                          self.navigationController?.pushViewController(homeVC, animated: true)
                            }else if user.type == "Admin"{
                                let homeVC = loadVC(storyboardMain, "DishListViewController") as! DishListViewController
                                                          self.navigationController?.pushViewController(homeVC, animated: true)
                            }else{
                                //do nothing...
                            }
